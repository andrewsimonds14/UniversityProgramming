...
void testJumble() {
    ...
    //My testing
    //Test accessors for JumblePuzzle
    cout << "Testing Accessors:" << endl;
    JumblePuzzle jpTest("ANDREW", "easy");
    showJumble(jpTest.getJumble(), jpTest.getSize());

    cout << "Testing Direction Accessor: (Should be n, s, e or w)  " << jpTest.getDirection() << endl;

    cout << "Testing Size Accessor: (Should be 12 (Twice Andrew))  " << jpTest.getSize() << endl;

    cout << "Testing Row PositionAccessor: (Should be random)  " << jpTest.getRowPos() << endl;

    cout << "Testing Column Position Accessor: (Should be random)  " << jpTest.getColPos() << endl;

    //Test Random Between function
    cout << "Testing Random Function: (Should be random int between 1->10)  ";
    int randomTest = 0;
    randomTest = JumblePuzzle::randomBetween(1, 10);
    cout << randomTest <<endl;

    //Test throwing of exceptions for invalid word or difficulty
    //Should throw BadJumbleException for invalid difficulty
    cout << "Testing difficulty exception:" << endl;
    try{
        JumblePuzzle jpTest2("ANDREW", "SUPER-HARD");
    }
    catch(BadJumbleException &e){
        cout << "Invalid difficulty: " << e.what() << endl;
    }

    //Should throw BadJumbleException for word size
    cout << "Testing word size constructor:" << endl;
    try{
        JumblePuzzle jpTest2("VERYLONGWORD", "easy");
    }
    catch(BadJumbleException &e){
        cout << "Invalid word: " << e.what() << endl;
    }
